...
public struct VaultConfig: Codable, Sendable {
  ...
  public static func create(password: String, iterations: Int = 600_000) throws -> (
    Self, SymmetricKey
  ) {
  ...
  public func unlock(password: String) throws -> SymmetricKey {
    guard format == "encrypted-folder", version == 1,
      ...
      (1_000...5_000_000).contains(iterations)
    else {
  ...
  private static func derivePasswordKey(_ password: String, salt: Data, iterations: Int) throws
    -> SymmetricKey
  {
...
public struct VaultCryptor: Sendable {
  ...
  public func encryptName(_ name: String, directoryID: Data) throws -> String {
    ...
    let box = try AES.GCM.seal(
      Data(name.utf8), using: nameKey(directoryID), authenticating: directoryID)
  ...
  private func encrypt(plainSize: UInt64, to destination: URL, read: (Int) throws -> Data) throws {
    ...
    let headerPrefix =
      Self.magic + fileID + plainSize.bigEndianData + UInt32(Self.chunkSize).bigEndianData
    let headerMAC = Data(
      HMAC<SHA256>.authenticationCode(for: headerPrefix, using: headerKey(fileID)))
...
public final class EncryptedFileReader: @unchecked Sendable {
  ...
  fileprivate init(url: URL, masterKey: SymmetricKey) throws {
    ...
    guard header.count == VaultCryptor.headerSize,
      ...
      UInt32(bigEndianData: header[28..<32]) == UInt32(VaultCryptor.chunkSize)
    else {
      try? handle.close()
      throw VaultError.damagedFile
    }
  ...
  private func decryptChunk(_ index: Int) throws -> Data {
    ...
    let encryptedOffset = UInt64(
      VaultCryptor.headerSize + index * (VaultCryptor.chunkSize + VaultCryptor.overhead))
    ...
    do {
      ...
      return try AES.GCM.open(
        box, using: fileKey, authenticating: header + UInt64(index).bigEndianData)
    } catch {
      throw VaultError.damagedFile
    }
  }
...
extension FixedWidthInteger {
  ...
  fileprivate init?(bigEndianData data: some DataProtocol) {
