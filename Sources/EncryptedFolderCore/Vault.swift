...
public final class Vault: @unchecked Sendable {
    ...
    public static func open(at url: URL, password: String) throws -> Vault {
        let configURL = url.appendingPathComponent(VaultConfig.fileName)
        guard let config = try? JSONDecoder().decode(VaultConfig.self, from: Data(contentsOf: configURL)) else {
            throw VaultError.invalidVault
        }
    ...
    public static func open(at url: URL, masterKeyData: Data) throws -> Vault {
        let configURL = url.appendingPathComponent(VaultConfig.fileName)
        guard let config = try? JSONDecoder().decode(VaultConfig.self, from: Data(contentsOf: configURL)),
              masterKeyData.count == 32 else {
            throw VaultError.invalidVault
        }
    ...
    public func items(in directory: URL) throws -> [VaultItem] {
        ...
        return try fileManager.contentsOfDirectory(
            ...
            guard url.lastPathComponent != VaultConfig.fileName,
                  ...
                  url.pathExtension != "partial" else { return nil }
    ...
    public func createFolder(named name: String, in parent: URL) throws -> VaultItem {
        ...
        do {
            ...
            return VaultItem(name: name, isDirectory: true, byteSize: nil, encryptedURL: destination, parentDirectoryID: parentID)
        } catch {
    ...
    public func rename(_ item: VaultItem, to newName: String) throws -> VaultItem {
        ...
        let destination = try availableEncryptedURL(for: newName, in: parent, directoryID: item.parentDirectoryID)
    ...
    public func move(_ item: VaultItem, into destinationDirectory: URL) throws -> VaultItem {
        if item.isDirectory,
           destinationDirectory.standardizedFileURL.path.hasPrefix(item.encryptedURL.standardizedFileURL.path + "/") {
        ...
        let destination = try availableEncryptedURL(for: item.name, in: destinationDirectory, directoryID: destinationID)
        try fileManager.moveItem(at: item.encryptedURL, to: destination)
        return VaultItem(
            name: item.name,
            isDirectory: item.isDirectory,
    ...
    private func directoryID(at url: URL) throws -> Data {
        ...
        let data = try Data(contentsOf: url.appendingPathComponent(Self.directoryMarker))
        guard data.count == 16 else { throw VaultError.damagedFile }
        return data
    }

    private func ensureAvailable(_ name: String, in directory: URL, excluding: VaultItem? = nil) throws {
        guard try !items(in: directory).contains(where: {
            $0.id != excluding?.id && $0.name.compare(name, options: [.caseInsensitive, .diacriticInsensitive]) == .orderedSame
        }) else {
            throw VaultError.alreadyExists
        }
    }

    private func availableEncryptedURL(for name: String, in directory: URL, directoryID: Data) throws -> URL {
        for _ in 0..<4 {
            let url = directory.appendingPathComponent(try cryptor.encryptName(name, directoryID: directoryID))
